...
let _Action = function (params) {
  ...
  var minimalKeys = ['id','xmlKey','townNum']; // absolutely needed, will trigger an error if absent
...
_Action.prototype.Finish = function() {
  ...
  // next 2 vars are needed because the "this" context is lost in .each
  var progresses = this.progress;
  var _action  = this;
  $(Object.keys(this.progress)).each(function(x,p) {
    var progress = progresses[p];
